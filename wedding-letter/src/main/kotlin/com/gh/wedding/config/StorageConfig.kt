...
class StorageConfig {
    ...
    fun s3Client(storageProperties: StorageProperties): S3Client {
        val credentialsProvider = resolveCredentialsProvider()
    ...
    private fun resolveCredentialsProvider(): AwsCredentialsProvider {
        val profileName = System.getenv("AWS_PROFILE")?.trim()?.takeIf { it.isNotBlank() } ?: "default"

        // 표준 우선순위: ~/.aws/credentials -> ~/.aws/config -> AWS 기본 체인
    ...
    private fun toStaticCredentialsProvider(
        ...
        log.info("AWS credentials loaded from {} profile={}", source, profileName)
